...
const dshInstall = require('./dsh-install');
 ...
 */
class Backend {
  constructor({ settings, logger, events, userDataDir }) {
  ...
  /** 把安装后的 dsh 入口写回 settings 并持久化，后续启动直接复用。 */
  persistDshBin(globalBin) {
    this.settings.dshBin = globalBin;
    this.log.info(`已将 dsh 入口写入 settings.dshBin: ${globalBin}`);
    if (this.userDataDir) {
      try {
        saveSettings(this.userDataDir, this.settings);
      } catch (e) {
        this.log.warn(`保存 dshBin 到 settings 失败: ${e.message}`);
      }
    }
  }

  /** 解析安装输出里的 npm 进度，推送给悬浮条/状态页。 */
  trackInstall(line) {
  ...
  async spawnOwn(port) {
    ...
    if (this.mock) {
      ...
      let dsh = resolveDsh(this.settings.dshBin);
      if (!dsh) {
        // 未找到本地 dsh：触发默认安装（先测速选最快的 npm 源 → npm 全局安装 → 回写 dshBin）
        dsh = await this.installDefaultDsh();
      }
      if (!dsh) return; // 安装失败已设置错误并调度重试
      if (dsh.kind === 'node') {
        cmd = this.nodeBin;
        cmdArgs = [dsh.bin, 'web', '--no-open', '--port', String(port)];
      } else {
        cmd = 'cmd.exe';
        cmdArgs = ['/d', '/s', '/c', `""${dsh.shim}" web --no-open --port ${port}"`];
      }
  ...
  /**
   * 默认安装 dsh（首次找不到本地 dsh 时的兜底）：
   * 先并行测速多个 npm 源、选「可访问且最快」的一个，再用 npm（而非 npx）全局安装
   * @deepseek-ai/dsh@latest，最后把入口路径写回 settings.dshBin 并持久化。
   * 成功返回解析后的 dsh 入口（{kind:'node', bin, source}），失败返回 null（已设置错误并调度重试）。
   */
  async installDefaultDsh() {
    this.installState = { phase: 'probing', detail: '正在测试 npm 镜像源，挑选最快的一个…', fetched: 0 };
    this.emitStatus();
    this.log.info('未找到本地 dsh 安装，将通过 npm 自动安装（先测速选最快源）');
    try {
      const fast = await dshInstall.pickFastestRegistry();
      this.log.info(
        `选定 npm 源: ${fast.name} (${fast.url}) ${fast.fallback ? '（测速全失败，回退默认源）' : `(${fast.ms}ms)`}`
      );
      this.installState = {
        phase: 'downloading',
        detail: `已选最快源 ${fast.name}（${fast.fallback ? '默认' : fast.ms + 'ms'}），开始安装 dsh…`,
        fetched: 0
      };
      this.emitStatus();

      const npmCli = dshInstall.resolveNpmCli(this.nodeBin);
      if (!npmCli) throw new Error('无法定位 npm（npm-cli.js）');
      const installEnv = {
        ...process.env,
      ...
      const res = await dshInstall.installDsh({
        registry: fast.url,
        nodeBin: this.nodeBin,
        npmCli,
        env: installEnv,
        onProgress: (line) => this.trackInstall(line)
      });
      if (res.code !== 0) {
        const tail = (res.err || '').split(/\r?\n/).filter(Boolean).slice(-3).join(' ');
        throw new Error(`npm 安装 dsh 失败 (exit=${res.code})${tail ? ': ' + tail : ''}`);
      }

      const globalRoot = await dshInstall.getNpmGlobalPrefix({ nodeBin: this.nodeBin, npmCli, env: installEnv });
      if (!globalRoot) throw new Error('无法确定 npm 全局安装目录（npm root -g 失败）');
      const globalBin = path.join(globalRoot, '@deepseek-ai', 'dsh', 'lib', 'bin.js');
      if (!fs.existsSync(globalBin)) throw new Error(`npm 安装完成但未找到 dsh 入口: ${globalBin}`);

      this.installState = { phase: 'finishing', detail: '安装完成，正在启动 dsh…', fetched: 0 };
      this.emitStatus();
      this.persistDshBin(globalBin);

      const resolved = resolveDsh(globalBin);
      if (!resolved) throw new Error('安装后 dsh 入口解析失败');
      return resolved;
    } catch (e) {
      this.installState = { phase: 'error', detail: e.message };
      this.state = 'error';
      this.error = `默认安装 dsh 失败: ${e.message}`;
      this.log.error(this.error);
      this.emitStatus();
      this.scheduleRetry();
      return null;
